...
export default async function StockComparisonPage({ params }) {
  const { slug } = params;
  const res = await client.getEntries({
    content_type: 'bondvoyage',
    'fields.slug': slug
  });
  const stockcomparison = res.items[0];
  if (!stockcomparison) return <p className="text-center mt-10">No Article</p>;

  const { date, author, title, body, image, summary, imagecredit } = stockcomparison.fields;
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-12">
      ...
      <div className="mb-8 space-y-2 [&>*]:!m-0">
        <p className="text-sm font-semibold text-blue-700 tracking-widest uppercase mb-2"> Stock Comparison </p>
