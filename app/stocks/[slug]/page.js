...
export default async function StocksPage({ params }) {
  const { slug } = params;
  const res = await client.getEntries({
    content_type: 'bondvoyage',
    'fields.slug': slug
  ...
  if (!stocks) return <p className="text-center mt-10">No Article</p>;

  const { author, title, body, image, summary, imagecredit } = stocks.fields;
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-12">
      ...
      <div className="mb-8 space-y-2 [&>*]:!m-0">
        <p className="text-sm font-semibold text-blue-700 tracking-widest uppercase mb-2"> Stocks </p>
